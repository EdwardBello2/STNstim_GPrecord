function data = readTimeSeriesData(timeseries, varargin)
%   D = LOADTIMESERIESDATA(TIMESERIES) loads all the data from the data field
%   of the timeseries object. D is of shape samples x electrodes.
%
%   D = LOADTIMESERIESDATA(TIMESERIES, INTERVALTIMES) loads the data field
...
%   
%
% Note: Based on the original code from the matnwb code
% util.loadTimSeriesData, but it was full of problems. I solved the
% problems (Ed Bello)

% To-Do:
% - phase out the "stride" function, never gonna use it
% - 
...
if isempty(electrode)
    ELEC = [1, dims(2)]; % currently this is broken, fix!!
    
elseif length(electrode) == 1
    ELEC = [electrode, electrode];
    
elseif length(electrode) == 2
    ELEC = [electrode(1), electrode(2)];
    
else
    error('electrode input incorrect')
    
end

if length(dims) == 1
    data = timeseries.data.load_mat_style(START:END);

elseif length(dims) == 2
    data = timeseries.data.load_mat_style(START:END,ELEC(1):ELEC(2));

else
    error('Dimensions > 2, ed hasnt planned for this yet!')
...
%         data(:,i) = readTimeSeriesData(timeseries, [], ...
...
%     if isa(timeseries.data, 'types.untyped.DataPipe')
%         data = timeseries.data.internal.load(START, STRIDE, END);
% 
%     else % usual case for normal TimeSeries objects
%         data = timeseries.data.load(START, STRIDE, END);
% 
%     end   
% %     disp('Done loading all !')
% 
