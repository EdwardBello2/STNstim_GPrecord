...
[psdSegsM, tstSegHarRef, fSegsM, resultsInfo] = func_gatherACCproc_allPSDTimeSegs_har(nwbM, cfg);
...
%%  Now detect frequency peaks over time
...
%     psd = smooth(psdDispM{iSeg}, 5);
    plot3(fSegsM{iSeg}, ones(length(psd), 1) * iSeg, psdDispM{iSeg}, ...
        'LineWidth', linWid, 'Color', cM(iSeg,:));
    
end
xlabel('Frequency (Hz)')
title('Uva')
set(ax(:), 'XLim', freqLim, 'View', view3d, ...
    'YTickLabel', [], 'YTick', [], ...
    'ZTickLabel', [], 'ZTick', [])
...
%     plot3(fSegsK{iSeg}, ones(length(psd), 1) * iSeg, psdDispK{iSeg}, ...
...
%     plot3(fSegsU{iSeg}, ones(length(psd), 1) * iSeg, psdDispU{iSeg}, ...
