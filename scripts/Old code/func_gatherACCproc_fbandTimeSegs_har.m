...
function [tremPow, tstSegHarRef, resultsInfo, TSTcleanCell] = func_gatherACCproc_fbandTimeSegs_har(nwb, cfg)
...
for iSeg = 1:nSegs
    [SaccCell{iSeg,1}, f, tSpecCell{iSeg,1}] = spectrogramacc3(accSeg{iSeg,1}, fs, cfg);
       
    % Also have tSpecCell (spectrogram time-midpoints) reflect in-file time
    tSpecCell{iSeg,1} = tstSeg{iSeg}(1) + tSpecCell{iSeg,1};
...
% for iSeg = 1:nSegs
[ScleanCell, TSTcleanCell, ...
    resultsInfo.artsRemovedBySegment] = removeMovArtInAll(SaccCell, tSpecCell, cfg);
...
[tremPow, tstSegHarRef, isTooSparse] = remSegsSparseData(tremPow, tstSegHarRef, cfg.tooSparseData);
resultsInfo.numDataSegments.tooSparse = sum(isTooSparse);

TSTcleanCell(isTooSparse) = [];

resultsInfo.numDataSegments.final = length(tremPow);
...
function [ScleanCell, TSTcleanCell, nRemovedBySegment] = removeMovArtInAll(SaccCell, tSpecCell, cfg)
...
for iSeg = 1:nSegs
    ...
    tSpecAll = [tSpecAll, tSpecCell{iSeg,1}];
end
...
for iSeg = 1:nSegs
    ...
    TSTcleanCell{iSeg,1} = TSTcleanAll(:,(begIdx(iSeg):endIdx(iSeg)));
