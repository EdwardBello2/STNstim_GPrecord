% script to display Spectrogram and PSDs of each recording on same scale,
% thanks to previously filling out all metadata into matlab table
...
load([projRootPath 'Data Acquisition\Kiwi Experiment\' 'RecordingsMetadata_selectDisplay.mat'], 'Tselect');
...
% popMaxPSD = max(Tselect.peakAmpPSD);
popMaxPSD = 25;



%%
for iRow = 1:nRows % first file of those recordings was just a test, skip
    close all
    clear Z S y
    ...
    load([projRootPath 'Data Acquisition\Kiwi Experiment\' fn], 'data', 'time', 'sampling_frequency', 'output_signal')
    ...
    % the slower, as much as possible. 
    
    % two frequencies used: 500 Hz, 8,193.4 Hz
    if fs > 500
        downFactor = 16;
        fs = fs/downFactor; % bring down to 512.0852
        dTime = downsample(t, downFactor);
        for i = 1:3
            y(:,i) = decimate(accFilt(:,i), downFactor);
    ...
    for ch = 1:3
%         ch = 1;
        [Z(:,:,ch), f, tSpec] = spectrogram(accFilt(:,ch), window, ...
            noverlap, [], fs, 'yaxis');
...
%% Add recently processed data to metadata table

% Tselect = [Tselect, table(samplingFrequency), table(harRefTime), table(totTime), table(movArtPerc), table(peakAmpPSD)];


% Save this result manually
