...
%%
for iRow = 2:nRows % first file of those recordings was just a test, skip
    close all
    clear Z S
...
%         ch = 1;
    ...
    ax.XLim = [0, 35];
    xlabel('Frequency (Hz)');
    title(['Minutes since harmaline inject: ' num2str(minutes(harRefTime(iRow)))], 'Interpreter', 'none')
...
T = [T, table(samplingFrequency), table(harRefTime), table(totTime), table(movArtPerc), table(peakAmpPSD)];
