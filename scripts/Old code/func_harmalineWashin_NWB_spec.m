...
function [bandPow_WashExpnorm, tmin] = func_harmalineWashin_NWB_spec(nwb, movingwin, cfg, lfpband, cpair, sessionMetaData)
...
% span for the smooth function, number of samples
span = 61; % see how this smoothing looks
...
for iEp = 1:nEps
    % First get spectrogram of particular LFP channel
%     movingwin = [5, 1];
    % note: t is in seconds, and based on the output of chronux mtspecgramc
    % tst is the timestamps from nwb file.
    [S,tspec,f,tst,~] = specgram_nwblfp(nwb, movingwin, cfg, ...
        'epoch', sessionMetaData.epochs{iEp}, ...
        'channel', cpair);
    
    
    % Next extract the band-specific total power over time from specgram
%     fband = [8, 12];
    idxfb = (f >= lfpband(1)) & (f < lfpband(2));
    Sband = S(:,idxfb);
    bandSum = sum(Sband, 2);
    
    
    % re-reference tspec by the earliest timestamp data from nwb file
    tspec = tspec + tst(1);
    tCell{iEp} = tspec;
    
    
    % Finally, smooth power-converted data and add to the cell
%     span = 31; % see how this smoothing looks
    bandPowCell{iEp} = smooth(pow2db(bandSum), span);
   
%     figure; plot(t, SbandSum)
...
for iEp = 1:nEps
    ...
    if strcmp('naivePreExp', sessionMetaData.epochs{iEp}) % if it's the baseline reference epoch
        % Trim the baseline portion so that it has only first 10 min of data,
        % and get the average power for that time% keep only the first 10 minutes of reference epoch (600 sec)
        [bandPowCell{iEp}, tCell{iEp}] = cropBaseline(bandPowCell{iEp}, ...
            tCell{iEp});
            
        bandPow_AvBaseline = mean(bandPowCell{iEp}); % initial naive condition
       
    end
        
    % Concatenate data
    bandPow_WashExp = [bandPow_WashExp; bandPowCell{iEp}];
    t = [t, tCell{iEp}];
...
% % Finally, normalize data to baseline condition level
% bandPow_WashExpnorm = bandPow_WashExp / bandPow_AvBaseline;

% In case I don't want to normalize:
bandPow_WashExpnorm = bandPow_WashExp;
...
function [bandPow_Baseline, t_Baseline] = cropBaseline(bandPow_Baseline, t_Baseline)
% bandPow_Baseline = bandPowCell{iEp};
% t_Baseline = tCell{iEp};

isfirstTen = t_Baseline < 480;
bandPow_Baseline = bandPow_Baseline(isfirstTen);
t_Baseline = t_Baseline(isfirstTen);

% bandPowCell{iEp} = bandPow_Baseline;
% tCell{iEp} = t_Baseline;
