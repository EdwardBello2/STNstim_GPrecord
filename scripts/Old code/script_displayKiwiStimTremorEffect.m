% script to see if tremors changed during DBS stim in Kiwi's data, based on
% "script_displayKiwiDualBands_freqAndHilbert.m"
...
%% obtain "baseline" tremor power (ideally pre-harmaline)
% based on the earliest recording of the day, 8 min post harmaline
...
% MEAN POWER FOR TOTAL TIME OF RECORDING, NO MOV ARTIFACTS INCLUDED
avLoMagSessionBaseline = mean(accLoMagSum);
avHiMagSessionBaseline = mean(accHiMagSum);
...
%     % title([titStr, ', Washin, dBgain (Washin/Naive)'])
    ...
    % get average pre-DBS portion for normalization
    isPre = (t >= 0) & (t < 15);
    isDBS = (t >= 15) & (t < 45);
    isPos = (t >= 45);
