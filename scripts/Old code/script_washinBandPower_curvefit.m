...
%% Plot time-series of selected band power washin over time

% First get total band power over time of washin data
fband = [12, 35]; % Hz
isFset = (f >= fband(1)) & (f < fband(2));
fset = f(isFset);
bandPow = sum(Swash(isFset,:), 1);


% Now normalize by total band power from aveage naive PSD
bandPowNorm = bandPow / sum(PSDbaseAv(isFset));


% remove any movement artifacts from power time series:
[~, isMovArt] = util.remoutliers(sum(bandPowNorm, 1), ...
    'bound', 'upper', ...
    'MADthresh', 4);
% temp = Sbase;
% temp(:,isMovArt) = [];
...
bandPowNormClean = bandPowNorm(~isMovArt);
tClean = t(~isMovArt);

bandPowDBgain = 10*log10(bandPowNormClean);
figure; plot(t/60, 10*log10(bandPowNorm));
hold on; plot(tClean/60, bandPowDBgain);


% fit the data using cftool:  X data: tClean;  Y data: bandPowDBgain
cftool

% % integrate power over frequency range for all spectrogram time windows
% fband = [12, 35]; % Hz
% isFset = (f >= fband(1)) & (f < fband(2));
% fset = f(isFset);
% WashNormFband = PSDWashNorm;
% WashNormFband(~isFset,:) = [];
% fbandPowerNorm = mean(WashNormFband,1);

% plot as scatterplot
% fbandGain = 10*log10(fbandPowerNorm);
% fbandGain = mean(10*log10(WashNormFband), 1);
% figure; scatter(t/60, fbandPowerNorm);
% figure; scatter(t/60, 10*log10(fbandPowerNorm));

% figure; scatter(t/60, bandPowNorm);
% figure; plot(t/60, bandPowNorm);
% figure; scatter(t/60, 10*log10(bandPowNorm));
% figure; plot(t/60, 10*log10(bandPowNorm));

% 
% 
% % fit line to it using cftool: 4 mg/kg case below
% a = 4.027; %  (2.719, 5.335)
% b = 0.01051; %  (0.003778, 0.01725)
% c = -274.8; %  (-367.1, -182.5)
% d = -1.535; %  (-2.822, -0.2479)
% x = -500:4000; % seconds
% y = (a./(1 + exp(-b*(x+c)))) + d; % average Gain
% 
% 
% % find 10%-90% rise time
% percRange = [min(y) + (0.1*a), max(y) - (0.1*a)];
% isInRange = (y >= percRange(1)) & (y < percRange(2));
% xInRange = x(isInRange);
% time10perc = min(xInRange);
% time90perc = max(xInRange);
% RiseTime = time90perc - time10perc;
