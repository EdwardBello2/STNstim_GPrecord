...
ACCdataPn = [dataAcqPn 'Mela\Harmaline\'];
SESSION_ID_prefix = 'harPilotMela-';


% Fullpath for directory in which to save NWB file results
nwbSavePn = [dataAcqPn 'NWBdata\MelaPilot\'];
nwbSuffix = '_ACCdata';
...
% add in data to the table for time since harmaline injection
...
for iFile = 1:nFiles
    ...
    dataTab = readtable([ACCdataPn fn]);
    fs = SessN.samplingFrequency(iFile); % hz
    sampNum = dataTab.AcquisitionAttempt; % acquisition tracking from G-link streaming, nice for tracking any missing samples. first sample is zero-index
    i_tst = (1/fs) * sampNum; % starting at time = 0 seconds; assuming that fs was constant throughout streaming
    i_acc = [dataTab.Channel1, dataTab.Channel2, dataTab.Channel3];
    i_acq = uint8(dataTab.AcquisitionAttempt);
