...
% To-Do
% -figure out how to make caching work for this
function [S, t, f, timestamps, fs] = specgram_nwblfp(nwb, varargin)
...
if cfg.allowCache
    ...
    matfnStr = [nwbName, 'mw', sprintf('-%d', movingwin), epochStr, ...
        'ch', sprintf('-%d', ch)];
...
%% Calculate spectrogram lfp data
...
elseif numel(ch) == 2
    ...
    [lfp2, timestamps, fs] = nwbLoadLfp(nwb, ...
        'channel', ch(2), ...
        'epoch', epochStr);
    ...
    error('I havent added functionality for more than two channels!')
...
if cfg.allowCache 
    save(fullCachefn, 'S', 't', 'f', 'timestamps', 'fs');
...
% dispSpecgram(t, f(fIdx), 10*log10(S(:,fIdx)'+eps))
